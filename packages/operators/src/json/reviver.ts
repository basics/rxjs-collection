/* eslint-disable @typescript-eslint/no-explicit-any */
import { map } from 'rxjs';

import type { Transform, AsyncTransform } from './replacer';

const asyncReviver: AsyncTransform<any, any>[] = [
  {
    validator: () => true,
    handler: () => source => source.pipe(map(value => value))
  }
];

export const syncReviver: Transform[] = [
  {
    validator: (value: any) => isValidUrl(value),
    handler: (value: string) => new URL(value)
  },
  {
    validator: (value: any) => isValidISODateString(value),
    handler: (value: string) => new Date(value)
  },
  {
    validator: (value: any) => isBigInt(value),
    handler: (value: string) => BigInt(value.slice(0, -1))
  },
  {
    validator: (value: any) => isRegExp(value),
    handler: (value: string) => regExpFromString(value)
  },
  {
    validator: (value: any) => isSymbol(value),
    handler: (value: string) => symbolFromString(value)
  },
  { validator: () => true, handler: (value: any) => value }
];

export function createSyncReviver<V = any, R = any>(transforms: Transform<V, R>[] = []) {
  return [...transforms, ...syncReviver];
}

export function createAsyncReviver<V = any, R = any>(transforms: AsyncTransform<V, R>[] = []) {
  return [...transforms, ...asyncReviver];
}

// export const createAsyncReviver = (
//   transforms: AsyncTransform[] = []
// ): AsyncTransform[] => [...transforms, ...asyncReviver];

const isValidUrl = (value: any): value is string =>
  isString(value) && URL.canParse(value) && /^[\w]+:\/\/\S+$/gm.test(value);

const isValidISODateString = (value: any): value is string => {
  if (typeof value !== 'string' || value.trim() === '') {
    return false;
  }
  const d = new Date(value);
  return !Number.isNaN(d.valueOf()) && d.toISOString() === value;
};

const isString = (value: any): value is string => value?.constructor === String;
const isBigInt = (value: any): value is string => isString(value) && /^\d+n$/.test(value);
const isRegExp = (value: any): value is string => isString(value) && /^\/.*\/[gimuy]*$/.test(value);
const isSymbol = (value: any): value is string =>
  isString(value) && /(\w?)Symbol\((\w+)\)/g.test(value);

const regExpFromString = (value: string): RegExp => {
  const match = value.match(/^\/(.*)\/([gimuy]*)$/);
  if (!match) {
    throw new Error(`Invalid RegExp string: ${value}`);
  }
  const [, pattern, flags] = match;

  if (!pattern) {
    throw new Error(`RegExp pattern cannot be empty: ${value}`);
  }

  // eslint-disable-next-line security/detect-non-literal-regexp
  return new RegExp(pattern, flags);
};

const symbolFromString = (value: string): symbol => {
  const match = /(?<prefix>\w?)Symbol\((?<name>\w+)\)/g.exec(value);
  if (!match || !match.groups) {
    throw new Error(`Invalid Symbol string: ${value}`);
  }
  const { prefix, name } = match.groups;

  if (!name) {
    throw new Error(`Symbol name cannot be empty: ${value}`);
  }

  if (prefix === 'g') {
    return Symbol.for(name);
  }
  return Symbol(name);
};
